...
namespace Utilities.Sql.SqlServer
{
    ...
    {
        public const string SYS_CREATED = "SysCreated";
        public const string SYS_CREATED_BY = "SysCreatedBy";
        public const string SYS_LAST_MODIFIED = "SysLastModified";
        public const string SYS_LAST_MODIFIED_BY = "SysLastModifiedBy";
...
SELECT
  DISTINCT
    ...
    PRIMARY_KEY_COLUMN = COALESCE(FK_CTE.primary_key_column, ''),
		IS_COMPUTED = CASE C.is_computed WHEN 0 THEN 'N' ELSE 'Y' END
  FROM
    sys.schemas AS S
  ...
  WHERE
    ...
    AND TBL.[name] = '{2}';";
            ...
            foreach (DataRow row in t.Rows)
            {
                ...
                if (nativeServerDataTypeName.NotEqualsCI("TIMESTAMP") 
									&& !row["IS_COMPUTED"].ToString().EqualsCI("Y")
									&& !IsSysGeneratedColumn(row["COLUMN_NAME"].ToString())
									&& !columnType.HasFlag(ColumnType.ID))
                ...
                this.Add(
                  new Column(table, row["COLUMN_NAME"].ToString())
                  {
                      ...
                      IsNullable = row["IS_NULLABLE"].ToString().EqualsCI("Y"),
											IsComputed = row["IS_COMPUTED"].ToString().EqualsCI("Y"),
...
SELECT
  ...
  WHERE
    TT.[name] = '{0}';";
        ...
        public List<String> GetStoredProcedureParameters(ColumnType columnType, IncludeKeyIdentificationComment includeKeyIdentificationComment = IncludeKeyIdentificationComment.Yes)
        {
            return
              ...
              .Where(column => (column.ColumnType & columnType) > 0
                && !IsSysGeneratedColumn(column.Name) && !column.IsComputed)
              .OrderBy(column => column.Name)
        ...
        public string GetWhereClausePrimaryKeyEqualsScopeIdentity(string alias = null)
        {
            var pk = this.Where(c => c.ColumnType.HasFlag(ColumnType.PrimaryKey))
                .Select(c => c.BracketedName)
                .Single();
            if (!string.IsNullOrWhiteSpace(alias))
            {
                pk = alias + "." + pk;
            }
            return string.Format("{0} = scope_identity()", pk);
        }
        ...
        public List<String> GetInsertColumnList()
        {
            return
              this
              .GetOrderedListBasedOnColumnType(ColumnType.CanAppearInInsertStatement)
              .Where(c => !IsSysGeneratedColumn(c.Name) && !c.IsComputed)
        ...
        public bool IsSysGeneratedColumn(string columnName)
        {
            if (String.IsNullOrWhiteSpace(columnName))
            {
                throw new ArgumentException("String was null or whitespace.", "columnName");
            }
            if (columnName.Equals(SYS_CREATED, StringComparison.OrdinalIgnoreCase)
                || columnName.Equals(SYS_CREATED_BY, StringComparison.OrdinalIgnoreCase)
                || columnName.Equals(SYS_LAST_MODIFIED, StringComparison.OrdinalIgnoreCase)
                || columnName.Equals(SYS_LAST_MODIFIED_BY, StringComparison.OrdinalIgnoreCase)
                )
            {
                return true;
            }
